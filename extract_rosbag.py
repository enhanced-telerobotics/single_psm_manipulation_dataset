...
Please see the import statements to figure out which packages are needed. Only works for python 2 because it relies directly on the rosbag package.
'''
...
for bagname in bag_list:
	...
	if output_format == 'video':
		try:
			video_out = cv2.cv.VideoWriter(out_filedir + '/capture.avi',cv2.cv.CV_FOURCC(*'MJPG'),30,(960,540))
		except:
			video_out = cv2.VideoWriter(out_filedir + '/capture.avi',cv2.VideoWriter_fourcc(*'MJPG'),30,(960,540))
	...
	if output_format == 'maxhz_labels':
		f = open(outfile_full + '/labels_maxhz.txt','w') # write the output file
	else:
		f = open(outfile_full + '/labels_30hz.txt','w') # write the output file
	# close the file
	f.close()
	# open in append mode
	if output_format == '200hz_labels':
		f = open(outfile_full + '/labels_maxhz.txt','a') # write the output file
	else:
		f = open(outfile_full + '/labels_30hz.txt','a') # write the output file
	...
	for topic,msg,t in bag.read_messages(topics = topic_dict.values()):
		...
		if topic == topic_dict['space_jacobian']:
			...
			if output_format == 'maxhz_labels':
				...
				tq_data,		# (3)
				pose_data,		# (7)
				pose_data_des,		# (7) NOT IN NEURAL NETWORK!
				vel_data,		# (6)
				joint_pos_data,
				jaw_pos_data,		# (7)
				joint_vel_data,
